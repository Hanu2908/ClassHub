...
subjects = [
    ...
    {"id": "5", "name": "Basic Electrical & Electronics Engineering / Basic Mechanical Engineering", "credits": 2},
    {"id": "6", "name": "Engineering Physics Lab / Engineering Chemistry Lab", "credits": 1},
    {"id": "7", "name": "Language Lab / Universal Human Values Lab", "credits": 1},
    {"id": "8", "name": "C Programming Lab", "credits": 1},
    {"id": "9", "name": "Basic Electrical & Electronics Engineering Lab / Manufacturing Practice Workshop", "credits": 1},
    {"id": "10", "name": "Computer Aided Engineering Graphics / Computer Aided Machine Drawing", "credits": 1.5},
    {"id": "11", "name": "Social Outreach, Discipline and Extra-Curricular Activities (SODECA)", "credits": 0.5}
]
...
def extract_numbers_resilient(str_val):
    # Extract numbers cleanly from the string, allowing grace/star marks (e.g. 85* -> 85)
    words = str_val.split()
    nums = []
    for w in words:
...
def get_marks_from_line_r2l(line):
    ...
    if len(rev) >= 3:
        ...
        # If they add up, total is 100% correct
        if ise + see == total:
            return total
        # If total is between 30 and 100, trust total
        if 30 <= total <= 100:
            return total
        # Fallback to sum if reasonable
        if 30 <= ise + see <= 100:
            return ise + see
            
    # Fallback to last number in normal order
...
def match_subject(line_text, sub_name):
    ...
    for option in options:
        ...
        # Get keywords, but allow shorter words (length >= 2) to capture AI, IT, ME, EE, CE
...
print("--- Testing R2L & Resilient Marksheet Parser ---")
matched_count = 0
matches_found = []
