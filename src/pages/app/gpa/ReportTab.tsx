        ...
        {manualSems.map(sem => {
          ...
          return (
            <div key={sem} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ fontSize: 12, color: val ? marksToColor(val * 10) : N.text, fontFamily: 'var(--font-mono)', width: 22, fontWeight: 600 }}>S{sem}</span>
              <input
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div id="gpa-report-card" style={{ background: '#0F111A', border: '1px solid #1E2235', borderRadius: 16, padding: 32, overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28 }}>
          <div>
            <div style={{ fontSize: 21, fontWeight: 800, color: '#FFFFFF', fontFamily: 'var(--font-display)', letterSpacing: '-0.03em' }}>ClassHub GPA Report</div>
            <div style={{ fontSize: 12, color: '#8B93A8', fontFamily: 'var(--font-mono)', letterSpacing: '0.05em', marginTop: 4 }}>SKIT · AUTONOMOUS · CBCS</div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 12, color: '#E2E8F0', fontFamily: 'var(--font-mono)', fontWeight: 600 }}>{activeBranch} Branch</div>
            <div style={{ fontSize: 12, color: '#8B93A8', fontFamily: 'var(--font-mono)', marginTop: 4 }}>{new Date().toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 24 }}>
          {[
            ...
            <div key={m.label} style={{ background: '#171A28', border: '1px solid #24293D', borderRadius: 12, padding: '16px 20px 14px' }}>
              <div style={{ fontSize: 12, color: '#94A3B8', fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 8 }}>{m.label}</div>
              ...
              <div style={{ fontSize: 12, color: '#64748B', fontFamily: 'var(--font-sans)', marginTop: 6, fontWeight: 500 }}>{m.sub}</div>
            </div>
          ))}
        </div>
          ...
          <div style={{ background: '#171A28', border: '1px solid #24293D', borderRadius: 12, overflow: 'hidden', marginBottom: 24 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', padding: '12px 20px', borderBottom: '1px solid #24293D', background: '#131521' }}>
              {['Semester', 'SGPA', 'Cumul. CGPA', 'Grade'].map(h => (
                <span key={h} style={{ fontSize: 12, fontWeight: 700, color: '#94A3B8', fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h}</span>
              ))}
            </div>
            {semHistory.map((d, i) => {
        ...
        <div style={{ background: '#131521', border: '1px solid #1E2235', borderRadius: 10, padding: '14px 20px', marginBottom: 20 }}>
          <p style={{ fontSize: 12, color: '#64748B', fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 10, fontWeight: 600 }}>SKIT Grading Scale</p>
          <div style={{ display: 'flex', gap: '8px 20px', flexWrap: 'wrap' }}>
            {GRADE_SCALE.map(g => (
              <span key={g.label} style={{ fontSize: 12, color: g.color, fontFamily: 'var(--font-mono)', fontWeight: 700 }}>{g.label}({g.point}) ≥{g.minMark}</span>
            ))}
          </div>
        </div>

        <div style={{ fontSize: 12, color: '#475569', fontFamily: 'var(--font-mono)', textAlign: 'center', fontWeight: 500 }}>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <button onClick={handleExport} disabled={exporting}
          style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '13px 16px', borderRadius: 'var(--radius-md)', background: exporting ? 'rgba(99,102,241,0.4)' : 'linear-gradient(135deg, #818CF8 0%, #6366F1 100%)', border: 'none', color: '#fff', fontSize: 15, fontWeight: 600, cursor: exporting ? 'not-allowed' : 'pointer', boxShadow: '0 4px 20px rgba(99,102,241,0.35)', transition: 'all 0.15s' }}>
          <Download size={15} />
          {exporting ? 'Exporting…' : 'Download PNG'}
        </button>
        <button onClick={handleShare}
          style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '13px 16px', borderRadius: 'var(--radius-md)', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--text-primary)', fontSize: 15, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' }}>
          <Share2 size={15} />
          Copy Link
        </button>
      </div>
