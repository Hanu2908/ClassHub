-- Migration: Clean counsellor notes on section deletion
-- Date: 2026-06-14
...
BEGIN
  ...
  -- 4. Delete counsellor notes for all students in this section
  DELETE FROM public.counsellor_notes
  WHERE student_id IN (SELECT id FROM public.users WHERE section_id = target_section_id);

  -- 5. Reset roles and section settings for all users in this section so they return to onboarding flow
  UPDATE public.users 
  SET 
    section_id = NULL,
    role = 'student',
  ...
  -- 6. Delete the section (which will cascade to subjects, timetable_slots, announcements, assignments, polls, section_teachers, attendance_sessions, etc.)
