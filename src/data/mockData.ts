...
// Backend prep: every export here maps 1:1 to a DB table / API endpoint.

// ── Types (duplicated here for mockData independence; source of truth = appStore) ──
export interface MockStudent {
  id: string;
  name: string;
  classRoll: string;        // 1-70 within section
  universityRoll: string;   // e.g. 25ESKCX089
}
...
// ── Mock class roster (sample 12 students for CR command center demo) ─────────
// Backend: SELECT * FROM hub_members WHERE hub_id = :hubId
export const mockStudents: MockStudent[] = [
  { id: 's01', name: 'Aarav Mehta',      classRoll: '01', universityRoll: '25ESKCX001' },
  { id: 's02', name: 'Bhavya Joshi',     classRoll: '02', universityRoll: '25ESKCX002' },
  { id: 's03', name: 'Chirag Rawat',     classRoll: '05', universityRoll: '25ESKCX005' },
  { id: 's04', name: 'Diksha Agarwal',   classRoll: '08', universityRoll: '25ESKCX008' },
  { id: 's05', name: 'Eshan Verma',      classRoll: '12', universityRoll: '25ESKCX012' },
  { id: 's06', name: 'Fatima Khan',      classRoll: '14', universityRoll: '25ESKCX014' },
  { id: 's07', name: 'Gaurav Singh',     classRoll: '16', universityRoll: '25ESKCX016' },
  { id: 's08', name: 'Harshita Gupta',   classRoll: '17', universityRoll: '25ESKCX017' },
  { id: 's09', name: 'Ishaan Patel',     classRoll: '21', universityRoll: '25ESKCX021' },
  { id: 's10', name: 'Jyoti Sharma',     classRoll: '24', universityRoll: '25ESKCX024' },
  { id: 's11', name: 'Karan Choudhary',  classRoll: '28', universityRoll: '25ESKCX028' },
  { id: 's12', name: 'Lavanya Mishra',   classRoll: '33', universityRoll: '25ESKCX033' },
];

// ── Mock class-wide attendance (per student) ──────────────────────────────────
// Backend: SELECT * FROM attendance WHERE hub_id = :hubId GROUP BY student_id
export const mockClassAttendance: StudentAttendanceRecord[] = mockStudents.map((st, i) => {
  const base = 70 + (i * 7 % 30); // vary 70-99
  const subjects = [
    { code: 'CH-101',  name: 'Engineering Chemistry',     type: 'Lecture',  present: Math.round(32 * base / 100), total: 32, percentage: base },
    { code: 'CS-304',  name: 'DBMS',                      type: 'Lecture',  present: Math.round(30 * (base - 5) / 100), total: 30, percentage: base - 5 < 0 ? 0 : base - 5 },
    { code: 'CS-306',  name: 'Operating Systems',          type: 'Lecture',  present: Math.round(26 * (base + 3) / 100), total: 26, percentage: Math.min(100, base + 3) },
    { code: 'CS-210',  name: 'AI Fundamentals',            type: 'Lecture',  present: Math.round(22 * base / 100), total: 22, percentage: base },
    { code: 'CS-318',  name: 'Database Lab',               type: 'Lab',      present: Math.round(16 * (base - 10) / 100), total: 16, percentage: base - 10 < 0 ? 0 : base - 10 },
    { code: 'ES-201',  name: 'Environmental Sciences',     type: 'Lecture',  present: Math.round(22 * (base - 20) / 100), total: 22, percentage: base - 20 < 0 ? 0 : base - 20 },
  ];
  const overall = subjects.reduce((acc, s) => acc + s.percentage, 0) / subjects.length;
  return { studentId: st.id, overall: Math.round(overall * 10) / 10, subjects };
});

// ── Mock submission tracker ───────────────────────────────────────────────────
// Backend: SELECT student_id FROM submissions WHERE assignment_id = :id
// Key: assignmentId → Set of studentIds who submitted
export const mockSubmissions: Record<string, string[]> = {
  'as1': ['s01', 's03', 's05', 's07', 's09', 's11'],  // 6/12 submitted
  'as2': ['s02', 's04', 's06', 's08'],                  // 4/12 submitted
  'as3': mockStudents.map(s => s.id),                   // all submitted
};

// ── Schedule ──────────────────────────────────────────────────────────────────
// Backend: SELECT * FROM schedule_slots WHERE hub_id = :hubId
...
export const mockSchedule: Record<string, {
  ...
  Thu: [
    ...
    { id: 's11n', day: 'Thu', subject: 'AI Fundamentals',       code: 'CS-210',  room: 'Block A-301', teacher: 'Dr. Singh',  type: 'Lecture',  startTime: '13:30', endTime: '15:00' },
    { id: 's11u', day: 'Thu', subject: 'Environmental Sciences',code: 'ES-201',  room: 'Block C-101', teacher: 'Dr. Patel',  type: 'Lecture',  startTime: '15:10', endTime: '16:10' },
  ],
  Fri: [
    ...
    { id: 's13n', day: 'Fri', subject: 'AI Fundamentals',       code: 'CS-210',  room: 'Block A-301', teacher: 'Dr. Singh',  type: 'Lecture',  startTime: '13:30', endTime: '15:00' },
    { id: 's13u', day: 'Fri', subject: 'Environmental Sciences',code: 'ES-201',  room: 'Block C-101', teacher: 'Dr. Patel',  type: 'Lecture',  startTime: '15:10', endTime: '16:10' },
  ],
  Sat: [
...
// ── Announcements ──────────────────────────────────────────────────────────────
// Backend: SELECT * FROM announcements WHERE hub_id = :hubId ORDER BY posted_at DESC
export const mockAnnouncements = [
  ...
  },
  {
    ...
    body: 'Technovanza 2026 registrations are open! Register before May 20. Forms available at the student union office and online.',
    priority: 'general' as const,
  ...
  },
  {
    ...
    body: 'Submit your AI/ML project proposal in IEEE format by May 24. Topics must be approved by Dr. Singh.',
    priority: 'general' as const,
...
// ── Assignments ──────────────────────────────────────────────────────────────
// Backend: SELECT * FROM assignments WHERE hub_id = :hubId ORDER BY created_at DESC
export const mockAssignments = [
  {
    ...
    hasSets: true,
    sets: [
      { id: 'set-a', label: 'Set A', rollStart: 1,  rollEnd: 25, pageNumbers: '4-5', description: 'Complete Pages 4–5.', pdfUrl: 'https://example.com/dbms-set-a.pdf' },
      { id: 'set-b', label: 'Set B', rollStart: 26, rollEnd: 50, pageNumbers: '6-7', description: 'Complete Pages 6–7.', pdfUrl: 'https://example.com/dbms-set-b.pdf' },
      { id: 'set-c', label: 'Set C', rollStart: 51, rollEnd: 70, pageNumbers: '8-9', description: 'Complete Pages 8–9.', pdfUrl: null },
    ],
    submittedLink: null,
  ...
  },
  {
    ...
    status: 'pending' as const,
    pdfUrl: null, hasSets: false, sets: [], submittedLink: null,
  ...
  },
  {
    ...
    pdfUrl: null, hasSets: false, sets: [],
...
// ── Polls ─────────────────────────────────────────────────────────────────────
// Backend: SELECT * FROM polls WHERE hub_id = :hubId ORDER BY created_at DESC
export const mockPolls = [
  {
    id: 'p1', question: 'Should campus network be upgraded to Wi-Fi 6E?',
  ...
  {
    id: 'p2', question: 'Who is attending the Project Expo?',
  ...
  {
    id: 'p3', question: 'Best time for extra classes on Saturdays?',
    type: 'anonymous' as const,
...
// ── Attendance ────────────────────────────────────────────────────────────────
// Backend: SELECT * FROM attendance WHERE student_id = :studentId AND hub_id = :hubId
